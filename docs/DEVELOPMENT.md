...
| 目錄 | 說明 |
|------|------|
| `example_hello` | 主選單提示，確認載入器運作 |
| `tw_ui_lexicon` | 台灣繁中介面詞彙覆寫（開啟後建議重啟） |
| `full_cg_unlock` | 解鎖全部 `TB_Picture` 照片圖鑑（靜默寫入物品欄） |

### 全 CG 說明

- **可以**：解鎖遊戲「照片／圖鑑」收集（`PutPicture` 那一套）。  
- **不保證**：劇情影片、Nutflex、貼圖／其他收集品、成就。  
- 需先有正常進度／存檔結構；純主選單可能尚未初始化物品欄。  
- 建議：開新局或讀檔後，到遊戲內按 F9 確認模組為「開啟中」，再進照片介面查看。
